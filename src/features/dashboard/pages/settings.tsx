...
import { GitConnectionsTab } from "@/features/dashboard/components/git-connections-tab";
import { NotificationChannelsTab } from "@/features/dashboard/components/notification-channels-tab";
...
export function SettingsPage() {
  ...
  const loadAll = async () => {
    ...
    else setAllRoles((r.data ?? []) as RoleRow[]);
    if (c.error) toast.error(c.error.message);
    else setClouds((c.data ?? []) as CloudRow[]);
    if (s.error) toast.error(s.error.message);
    else setSources((s.data ?? []) as SourceRow[]);
    if (k.error) toast.error(k.error.message);
    else setSshKeys((k.data ?? []) as SshRow[]);
    setLoading(false);
  };
  ...
  return (
    <div>
      <PageHeader
        title="Settings"
        description="Manage your workspace, integrations and access control."
      />
      ...
      <Tabs defaultValue="users" className="w-full">
        <TabsList className="bg-card/60">
          ...
          <TabsTrigger value="git">Git connections</TabsTrigger>
        ...
        <TabsContent value="roles" className="mt-4">
          <Card className="bg-card/60">
            <CardHeader>
              ...
              <CardDescription>
                {isAdmin
                  ? "Grant or revoke admin, moderator and user roles."
            ...
            <CardContent className="space-y-2">
              {profiles.map((p) => {
                ...
                return (
                  <div
                    key={p.id}
                    className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border/50 bg-muted/20 px-4 py-3"
                  >
                    <div className="min-w-0">
                      <p className="truncate text-sm font-medium">
                        {p.display_name ?? p.email ?? p.id}
                    ...
                    {isAdmin && (
                      <RoleAssigner
                        onAssign={(role) => assignRole(p.id, role)}
          ...
          <Card className="bg-card/60">
            <CardHeader className="flex flex-row items-start justify-between gap-3">
              <div>
                ...
                <CardDescription>
                  Connect one or more deployment targets. Each needs a unique name.
                </CardDescription>
              </div>
              {isAdmin && (
                <CloudFormDialog
            ...
            <CardContent className="grid gap-2 sm:grid-cols-2">
              {clouds.length === 0 && (
                <p className="text-sm text-muted-foreground">No cloud providers yet.</p>
              )}
              {clouds.map((c) => (
                <div
                  key={c.id}
                ...
                >
                  ...
                  {isAdmin ? (
                    <CloudFormDialog
                      ...
                      onSaved={loadAll}
                      trigger={
                        <Button size="sm" variant={c.connected ? "outline" : "default"}>
                          {c.connected ? "Manage" : "Connect"}
                        </Button>
                      }
                    />
                  ) : (
                    <Button size="sm" variant={c.connected ? "outline" : "default"} disabled>
                      {c.connected ? "Manage" : "Connect"}
                    </Button>
                  )}
                </div>
          ...
          <Card className="bg-card/60">
            <CardHeader className="flex flex-row items-start justify-between gap-3">
              <div>
                ...
                <CardDescription>
                  Connect Git providers. Each connection needs a unique name.
                </CardDescription>
              </div>
              {isAdmin && (
                <SourceFormDialog
            ...
            <CardContent className="grid gap-2 sm:grid-cols-2">
              {sources.length === 0 && (
                <p className="text-sm text-muted-foreground">No source connections yet.</p>
              )}
              {sources.map((s) => (
                <div
                  key={s.id}
                ...
                >
                  ...
                  {isAdmin ? (
                    <SourceFormDialog
                      ...
                      onSaved={loadAll}
                      trigger={
                        <Button size="sm" variant={s.connected ? "outline" : "default"}>
                          {s.connected ? "Manage" : "Connect"}
                        </Button>
                      }
                    />
                  ) : (
                    <Button size="sm" variant={s.connected ? "outline" : "default"} disabled>
                      {s.connected ? "Manage" : "Connect"}
                    </Button>
                  )}
                </div>
        ...
        {/* GIT CONNECTIONS — Encore-backed */}
        <TabsContent value="git" className="mt-4">
          <GitConnectionsTab isAdmin={isAdmin} />
        </TabsContent>

        {/* NOTIFICATION CHANNELS — Encore-backed */}
        <TabsContent value="channels" className="mt-4">
          <NotificationChannelsTab isAdmin={isAdmin} />
        </TabsContent>
      </Tabs>
    </div>
  );
      ...
      <DialogContent>
        ...
        <div className="space-y-3">
          <div className="space-y-1.5">
            <Label>Name</Label>
            <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="prod-aws" />
          </div>
          <div className="space-y-1.5">
...
function SshFormDialog({
  ...
  const submit = async () => {
    if (!name.trim() || !fingerprint.trim())
      return toast.error("Name and fingerprint are required");
    setSaving(true);
    const payload = {
      name: name.trim(),
